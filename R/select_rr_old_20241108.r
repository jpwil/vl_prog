...
select_rr <- function(data, keep = NULL, p_select = 0.1) {
  if (is.null(data$mice$result)) {
    message("\nMultiple imputation not available")
    return(NULL)
  }
  ...
  cluster <- "STUDYID"
  predictors_init <- c("DM_SEX", "DM_AGEs", "ZZ_AGEs2", "ZZ_AGEs3", "TREAT_GRP4", "ZZ_MAL", "LB_BL_HGB_GRP2", "VL_HISTORY", "VL_DURATIONs", "LB_BL_WBCs", "LB_BL_PLATs", "LB_BL_ALTs", "LB_BL_CREATs", "MP_BL_SPLEEN_LENGTHs2", "MB_COMBINEDs")
  cat <- c("TREAT_GRP4", "ZZ_MAL", "LB_BL_HGB_GRP2")
  ...
  i <- 1
  stop <- FALSE
  data_original <- data$mice$result_grp
  
  while (!stop) {
    cat("\n***************************\n** Starting while loop", i, "**\n***************************\n\n")
    models_fit <- list()
    for (j in 1:data_original$m) {
      cat("Full imputation:", j, "\n")
      models_fit[[j]] <- data_original %>% 
        complete(j) %>%  
        glmer(formula = formula_new, family = binomial) 
    }
    ...
    if (any(grepl("^TREAT_GRP4", model_terms))) {
      ...
      for (j in 1:data_original$m) {
        ...
        models_fit_comp[[j]] <- data_original %>% 
          complete(j) %>% 
          glmer(formula = formula_temp, family = binomial) 
      }
    ...
    if (any(grepl("^ZZ_MAL", model_terms))) {
      models_fit_comp <- list()
      formula_temp <- update(formula_new, . ~ . - ZZ_MAL) # need to fit model without categorical variable
      for (j in 1:data_original$m) {
    ...
    # reduced model for D1 multivariate Wald test - LB_BL_HGB_GRP2
    if (any(grepl("^LB_BL_HGB_GRP2", model_terms))) {
      models_fit_comp <- list()
      formula_temp <- update(formula_new, . ~ . - LB_BL_HGB_GRP2) # need to fit model without categorical variable
      for (j in 1:data_original$m) {
        cat("LB_BL_HGB_GRP2 multivariate Wald model:", j, "\n")
        models_fit_comp[[j]] <- data_original %>% 
          complete(j) %>% 
          glmer(formula = formula_temp, family = binomial) 
      }
      comparison <- D1(models_fit, models_fit_comp)
      add_row <- data.frame(term = "LB_BL_HGB_GRP2", p.value = summary(comparison)$comparisons$p.value)
      select_table <- rbind(select_table, add_row)
      select_table <- select_table %>% filter(!term %in% c("LB_BL_HGB_GRP2Moderate", "LB_BL_HGB_GRP2Mild_norm"))
    }
    ...
    if (highest_p[[2]] > p_select) {
      ...
      cat("Removing predictor: ", highest_p[[1]], ", with p-value of: ", highest_p[[2]], "\n", sep = "")
...
safe_select_rr <- function(data, keep, p_select) {
  ...
  tryCatch(
    {
      withCallingHandlers(
        {
          ...
          result <- select_rr(data, keep, p_select)
      ...
      message("Completed execution of select_rr.")
    }
  )
}
